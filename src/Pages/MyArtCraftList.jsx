...
const MyArtCraftList = () => {
  ...
  useEffect(() => {
    if (user?.email) {
      fetch(`http://localhost:5000/allArtCraft/${user.displayName}`)
        .then(response => response.json())
        .then(data => {
          setItems(data);
          console.log(data);
        })
        .catch(error => console.error('Error fetching art & craft items:', error));
    }
        ...
        <tbody className="bg-white divide-y divide-gray-200">
          {items.map(item => (
            <tr key={item._id}>
              ...
              <td className="px-6 py-4 whitespace-nowrap">{item.shortDescription}</td>
              <td className="px-6 py-4 whitespace-nowrap">{item.userEmail}</td>
              <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                <Link to={`/view-details/${item._id}`} className="text-indigo-600 hover:text-indigo-900">View Details</Link>
              </td>
            </tr>
          ))}
        </tbody>
